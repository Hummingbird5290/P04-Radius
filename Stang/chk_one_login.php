...
function UDataCHK($name,$connect_db) {
	...
	$query_sum_t = "
SELECT 
sum(date_end) as tcard
FROM
  table_usergroup_temp
   where   UserName='$name'       and  active='0'   ";
...
//อัพเดท เวลาที่ออนไลน์
$RadAcctId=$row_setingz[RadAcctId];
$Ponline=date('Y-m-d H:i:s');
mysql_query( "update  radacct  set updates  ='$Ponline'    where  RadAcctId='$RadAcctId' and     AcctStopTime='0000-00-00 00:00:00'  ");


#บล๊อคแมค
$mac=$row_setingz[CallingStationId];
...
if ($totalRows_mac != 0) {
	print  "<IFRAME name=\"IFRAME\"   src=\"http://$SERVERNAMES:$VLPORT/logoff \"   width=\"300\" height=\"130\" frameborder=\"0\" scrolling=\"yes\"></IFRAME>";

}

//กรณ๊ fix mac กับ id ที่ Login ไว้
 
$mac2=$_GET[mac]; //mac ที่ login
$block_mac_sql = "SELECT   *   FROM    radcheck    where  UserName='$UserName'   and  Attribute='Calling-Station-Id'  ";
$mac_db = mysql_query($block_mac_sql, $connect_db) or die(mysql_error());
$row_fix_mac = mysql_fetch_assoc($mac_db);
$totalRows_mac = mysql_num_rows($mac_db);
if ($totalRows_mac == 1) { //มีการกำหนด ฟิก หมายเลข mac

          if($row_fix_mac[Value] != $mac2){
...
{
	print  "<IFRAME name=\"IFRAME\"   src=\"http://$SERVER_NAME:$VLPORT/logoff \"   width=\"300\" height=\"130\" frameborder=\"0\" scrolling=\"yes\"></IFRAME>";
...
if(  $totalRows_mac  != "1" )
{
	##############  ´Õ´ user ÍÍ¡¨Ò¡à¹çµ ËÒ¡ à¤éÒµèÍÍ



	print  "<IFRAME name=\"IFRAME\"   src=\"http://$SERVER_NAME:$VLPORT/logoff \"   width=\"300\" height=\"130\" frameborder=\"0\"
...
if($totalMax_All_Session >0){
	...
	{
		mysql_query("delete   from   radcheck  where  UserName='$UserName'     and  Attribute='Expiration'  ") or die(mysql_error());

		mysql_query("INSERT INTO  radcheck  (UserName,Attribute,op,Value)   values('$UserName','Expiration',':=','5 Mar 2008 01:00:00')")
		or    die ("Add ข้อมูลลง Table ไม่ได้ ") or die(mysql_error());
		mysql_query("delete   from   radcheck  where  UserName='$UserName'     and  Attribute='Max-All-Session'  ") or die(mysql_error());
		mysql_query("update       radacct  set AcctSessionTime='0'  where  UserName='$UserName'        ") or die(mysql_error());
...
scrolling=\"yes\"></IFRAME>";
	...
	$select_sql = "SELECT  sum(AcctSessionTime) as alltime FROM  radacct       WHERE UserName='$UserName'
                                        and ( AcctSessionTime !=''  and  AcctSessionTime !='0'  and  AcctSessionTime !='0000-00-00 00:00:00' ) ";
	...
	$select_sql = "SELECT  *  FROM  radacct       WHERE UserName='$UserName'
                                        and ( AcctStopTime =''  or  AcctStopTime ='0'  or  AcctStopTime ='0000-00-00 00:00:00' )  order  by  RadAcctId desc";
	$select_sql_db = mysql_query($select_sql, $connect_db) or die(mysql_error());
	$row_select_sql= mysql_fetch_assoc($select_sql_db);
	...
	if($row_All_Session[Value]<$cal_date)   //หากเวลาในระบบ น้อยกว่าเวลาที่ใช้ให้เด้งออก
	{
		...
		print  "<IFRAME name=\"IFRAME\"   src=\"http://$SERVERNAMES:$VLPORT/logoff \"   width=\"300\" height=\"130\" frameborder=\"0\"
scrolling=\"yes\"></IFRAME>";
	} else

	{
		...
		//date('s',$alltime)
		$rty= (strtotime($date_now) - strtotime($row_select_sql[AcctStartTime]) );
