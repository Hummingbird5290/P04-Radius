...
<body>
<form name="form1" method="post" action="index.php?case_i=73">
  <?
...
$txt_search= preg_replace($pattern, $replacement, $txt_search);
		...
		$sql = "SELECT  *      FROM       card_true     where    domain='$domian_name'   
										   $condition    ";
		...
		$Show = $sql." order by TF desc,date_add desc  LIMIT $offset, $limit";
		$query_data = $db->query($Show);

?>
  <table width="85%" border="0" align="center" cellpadding="1" cellspacing="1"  class="imagetable">
    <tr bgcolor="#FFFFFF"> 
      <th height="30" colspan="6"> <div align="right"><strong>ค้นหารหัสบัตรทรู</strong> 
          <input name="txt_search" type="text" value="<?=$txt_search;?>" size="40">
          <input type="submit" name="Submit" value="  ค้นหา  " class="styled-button-2"  >
        </div></th>
    </tr>
    <tr bgcolor="#CCFFCC"> 
      <th colspan="6"> <div align="center" class="style1 style2"><font size="3" face="MS Sans Serif, Tahoma, sans-serif"><strong><img src="logo_truemoney.png" width="157" height="40"><br>
รายการบัตรที่ทำการ Active</strong></font></div></th>
  ...
  if($rows<>0){
    ...
    <tr bgcolor="<?php echo "$cli"; ?>" > 
      ...
      <td><div align="center"><font size="2" face="MS Sans Serif, Tahoma, sans-serif">
          <?php     if($result_data[TF]=="1")              {  print 'โอนเงินเข้าบัญชีแล้ว';  }  
		         if($result_data[TF]=="0" or $result_data[TF]=NULL)              {  print 'รอโอนเงินเข้าบัญชี';  }  
 ?>
      </font></div></td>
      <td><div align="right"><font size="2" face="MS Sans Serif, Tahoma, sans-serif"> 
          <?
		    if($result_data[TF]=="0" or $result_data[TF]=NULL)              {  $TU_TOTAL=number_format($result_data[cost],2)+  $TU_TOTAL;  } print  number_format($result_data[cost],2 );   ?>
          </font></div></td>
    </tr>
    <?
			$i++;
...
<br>
 <div align="right">
  ...
  <tr>
          ...
          <td bgcolor="#6699CC"><div align="center">รวม <?php print number_format($TU_TOTAL,2);?> &nbsp; บาท</div> </td>
    </tr>
  </table>
<? if($rows > 0){ ?>
...
echo   "<a href='index.php?case_i=73&offset=$prevoffset&txt_search=$txt_search'>
<font  color=\"red\"><< ก่อนหน้า</font></a>\n\n";
    ...
    for ($i=1;$i<=$pages;$i++) { 
        ...
        if (($offset/$limit) == ($i-1)) { 
            ...
            $newoffset=$limit * ($i-1); 
                  echo  "<a href='index.php?case_i=73&offset=$newoffset&txt_search=$txt_search'     >
