...
if($domain_name=='mitsix'   or  $domain_name=='tabsala' ) {
	...
	if($domain_name !='mitsix') {
		 ...
		 if($_GET[domain_name] !='huzzun') {
		       echo "<meta http-equiv=refresh content=0;URL=http://www.thaigqsoft.info/active.php?domain_name=$_GET[domain_name]>&per=$UserName"; 
			   }
			   else {
			     echo "<meta http-equiv=refresh content=0;URL=http://www.thaigqsoft.info/huzzun_card_active.php?domain_name=$_GET[domain_name]>&per=$UserName"; 
			   }
		 	exit();  
...
$query_user = "select  *  from register     where               per='$strID'                      ";
...
{   //ตรวจสอบว่า มี userlogin นี้อยุ่หรือไม่
            echo                  "<script type=text/javascript>";
		    echo                  "alert('มีการใช้เลขประจำตัวประชาชน $strID นี้แล้วใน ระบบ ')";
	        echo                  "</script>";
		        echo "<meta http-equiv=refresh content=0;URL=user_register_from_mikrotik_wat.php?domain_name=$domain_name>"; 
		 	exit();
}  //ตรวจสอบว่า มี userlogin นี้อยุ่หรือไม่
...
#sms เสียข้ามขั้นตอนยืนยันไปก่อน
    echo "<meta http-equiv=refresh content=0;URL=user_register_from_mikrotik_wat.php?domain_name=$domain_name&uhz=OK&UserName=$UserName&password=$password>"; 
exit();
?>
<hr>
<form name="chkmobile"  method="post" action="user_register_from_mikrotik_wat.php?domain_name=<?php print  $domain_name;?>">
...
<form action="user_register_from_mikrotik_wat.php?domain_name=<?php print $_GET['domain_name'];?>" method="post" enctype="multipart/form-data" name="form1" onSubmit="return chkIDCard()">
