...
<body>
<form name="form1" method="post" action="index.php?case_i=68">
  <?
...
$txt_search= preg_replace($pattern, $replacement, $txt_search);
			$condition = "   and       userdb_user     like  '%$txt_search%'       ";
		}
		...
		$sql = "SELECT *  FROM   login_pass    where userdb_user<>'tlog'   and domain='$domain_name'   $condition";
...
?>
  <table width="800" border="0" align="center" cellpadding="1" cellspacing="1"  class="imagetable">
    <tr bgcolor="#FFFFFF"> 
      <th height="30" colspan="7"> <div align="right"><strong>ค้นหาชื่อผู้ใช้</strong> 
          <input name="txt_search" type="text" value="<?=$txt_search;?>" size="40">
          <input type="submit" name="Submit" value="  ค้นหา  " class="styled-button-2"  >
        </div></th>
    </tr>
    <tr bgcolor="#CCFFCC"> 
      <th colspan="7"> <div align="center" class="style1 style2"><font size="3" face="MS Sans Serif, Tahoma, sans-serif"><strong>รายงานเข้าระบบแอดมิน</strong></font></div></th>
    </tr>
    <tr bgcolor="#FFFFCC"> 
      <th width="11%"> <div align="center" class="style3"><strong><font size="2" face="MS Sans Serif, Tahoma, sans-serif">ลำดับ</font></strong></div></th>
      <th width="23%" align="center"><strong><font size="2" face="MS Sans Serif, Tahoma, sans-serif">ชื่อเข้าใช้</font></strong></th>
      <th width="23%" align="center"><strong><font size="2" face="MS Sans Serif, Tahoma, sans-serif">เวลาเข้าใช้</font></strong></th>
      <th width="19%" align="center"><strong><font size="2" face="MS Sans Serif, Tahoma, sans-serif">ip</font></strong></th>
	      <th width="24%" align="center"><strong><font size="2" face="MS Sans Serif, Tahoma, sans-serif">Track IP</font></strong></th>
    </tr>
    <?
  if($rows<>0){
	  $i=1;
	  while($result_data = mysql_fetch_array($query_data)){
	//  $check=$db->query("  SELECT *  FROM   login_pass  where userdb_user<>'yim'  $condition ");
	  	    $cli=$i%2;
    ...
    <tr bgcolor="<?php echo "$cli"; ?>" > 
      ...
      <td><font size="2" face="MS Sans Serif, Tahoma, sans-serif"><?  print   $result_data[ip];   ?></font></td>
...
echo   "<a href='index.php?case_i=68&offset=$prevoffset&txt_search=$txt_search'>
<font  color=\"red\"><< ก่อนหน้า</font></a>\n\n";
    ...
    for ($i=1;$i<=$pages;$i++) { 
        ...
        if (($offset/$limit) == ($i-1)) { 
            ...
            $newoffset=$limit * ($i-1); 
                  echo  "<a href='index.php?case_i=68&offset=$newoffset&txt_search=$txt_search'     >
